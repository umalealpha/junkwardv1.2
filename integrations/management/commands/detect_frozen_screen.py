...
titles or image URLs stored or printed (AD-POL-AI-GOV-001).
  ...
  python manage.py detect_frozen_screen --user "Meduduetso" --date 2026-08-29 --days 90
...
# Time Doctor's /api/1.0/files truncates a whole-company pull (a 113-person day
# came back with only 42 people's screenshots), so a single call silently MISSES
# staff. Pull in small user batches and combine, so the sweep covers everyone.
_USER_BATCH = 10
    ...
    def _one_day(self, client, users, call_ids, day, *, focus, persist=False):
        ...
        for s in rows:
            self.stdout.write(
                ...
                f"({s.frozen_typing_hours:.1f}h) mouse_dead={s.mouse_dead_pct*100:>4.0f}% "
    ...
    def _backscan(self, client, users, call_ids, end_day, days):
        ...
        for i in range(days):
            ...
            self.stdout.write(
                ...
                f'{s.frozen_typing_hours:>6.1f} {s.mouse_dead_pct*100:>9.0f}% '
