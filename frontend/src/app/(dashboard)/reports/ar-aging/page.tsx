...
import { downloadReportXlsx, getArAging, getToken } from '@/lib/api'
...
export default function ArAgingPage() {
  ...
  const handleGenerate = async () => {
    ...
    setError(null)
    try {
      const data = await getArAging(asOf, companyId)
      setReport(data)
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Failed to generate AR aging report')
    } finally {
      setLoading(false)
    }
  }
  ...
  const handleExport = () => {
    ...
    exportToCsv(rows, `ar_aging_${asOf}`)
  }
          ...
          report && (
            <div className="flex items-center gap-2">
              ...
              <Button
                ...
                size="sm"
                leftIcon={<Download className="w-3.5 h-3.5" />}
                onClick={() => downloadReportXlsx('ar_aging', {
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead className="bg-[#F3F4F6] border-b-2 border-[#E5E7EB]">
                    <tr>
                      <th className="px-4 py-3 text-left text-xs font-semibold text-[#374151] uppercase tracking-wider">Customer</th>
